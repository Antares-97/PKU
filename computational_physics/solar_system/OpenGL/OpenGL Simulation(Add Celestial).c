...
void Display()
{
    ...
    glLoadIdentity();
    //glOrtho(-3*10.2*AU,3*10.2*AU,-3*6.1*AU,3*6.1*AU,3*AU,-3*AU);                          //调用正投影视角
    ...
    glPushMatrix();
    glColor3ub(157, 17, 243);
    glutSolidTorus(Re/2,5.202*AU,10,100);
    glTranslatef(solar_system[5]->x, solar_system[5]->y, 0.0f);
    glutSolidTorus(Re/2,AU,10,100);
    glutSolidSphere(11.19*Re/3,10,10);
    glPopMatrix();
    
    glPushMatrix();
    glColor3ub(198, 194, 68);
    glutSolidTorus(Re/2,9.544*AU,10,100);
    glTranslatef(solar_system[6]->x, solar_system[6]->y, 0.0f);
    glutSolidTorus(Re/2,AU,10,100);
    glutSolidSphere(9.41*Re/2,10,10);
    glPopMatrix();
    
    glPushMatrix();
    glColor3ub(44, 187, 246);
    glutSolidTorus(Re/1.5,19.19*AU,10,100);
    glTranslatef(solar_system[7]->x, solar_system[7]->y, 0.0f);
    glutSolidTorus(Re/1.5,1.5*AU,10,100);
    glutSolidSphere(4.01*Re/1,10,10);
    glPopMatrix();
    
    glPushMatrix();
    glColor3ub(61, 19, 226);
    glutSolidTorus(Re/1,30.13*AU,10,100);
    glTranslatef(solar_system[8]->x, solar_system[8]->y, 0.0f);
    glutSolidTorus(Re/1,2*AU,10,100);
    glutSolidSphere(3.88*Re/1,10,10);
    glPopMatrix();
...
void create_solar_system()
{
    ...
    printf("Please input the new planet's mass/me:\n");                             //新星体的质量
    scanf("%le",&mass);
    printf("Input the new planet's radius/Re:\n");                                       //半径
    scanf("%le",&radius);
    printf("Input the new planet's coordination x/AU\n");                          //x坐标初始值
    scanf("%le",&x);
    printf("y/AU:\n");                                                                                   //y坐标初始值
    scanf("%le",&y);
    printf("Input the new planet's velocity vx:\n");                                     //vx初始值
    scanf("%le",&vx);
    printf("vy:\n");                                                                                        //vy初始值
    scanf("%le",&vy);
    ...
    solar_system[10]=createPlanet("newplanet", mass*me, radius*Re, x*AU, y*AU, vx, vy);         //设置一个新的星体
}
